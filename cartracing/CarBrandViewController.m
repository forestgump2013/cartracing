...
{
    if ([brandTable respondsToSelector:@selector(setSeparatorInset:)]) {
        [brandTable setSeparatorInset:UIEdgeInsetsMake(0, 0, 0, 0)];
    }
    if ([brandTable respondsToSelector:@selector(setLayoutMargins:)]) {
        [brandTable setLayoutMargins:UIEdgeInsetsMake(0, 0, 0, 0)];
    }
}
...
{
    NSLog(@"table did select is called! level=%d",(int)level);
    ...
    switch (level) {
       ...
       //     NSLog(brand);
        ...
        case 1:
        {
            currentProducer=[keys objectAtIndex:seciton];
            NSMutableArray *subList=[lists objectAtIndex:seciton];
            currentSerice=[subList objectAtIndex:row];
          //  NSString *brand=[subList objectAtIndex:row];
        ...
        case 2:
        {
            ...
            [nav popViewControllerAnimated:NO];
            [nav popViewControllerAnimated:NO];
            CreateNewCarViewController *superView=(CreateNewCarViewController*)[nav topViewController];
            [superView setBrandName:[[NSString alloc] initWithFormat:@"%@ %@ %@",currentBrand,currentSerice,oilUnit_model] markName:[brandDictionary objectForKey:currentBrand]];
