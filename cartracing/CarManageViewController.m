...
@synthesize oneCarLicense,oneCarInspectDate,oneCarInsuranceDate;
//@synthesize oneCarMiles;
//@synthesize oneCarBrandMark;
//@synthesize oneCarOilBoxVolume;
//@synthesize oneCarBrandName;
@synthesize deleteCarBtn,inspectValBtn,insuranceValBtn;
...
{
    CGRect frame=CGRectMake(inspectValBtn.frame.origin.x, inspectValBtn.frame.origin.y-40, 70, 160);
    [areaViewController initWithData:appDelegate.carCheckYearUnits Frame:frame Button:inspectValBtn];
...
{
    CGRect frame=CGRectMake(insuranceValBtn.frame.origin.x, insuranceValBtn.frame.origin.y-40, 70, 160);
    [areaViewController initWithData:appDelegate.carCheckYearUnits Frame:frame Button:insuranceValBtn];
    [self.view addSubview:areaViewController.view];
}
...
{
    newItem.itemId=[editingCarMaintainItems count]+21;
    newItem.carId=editCar.car_id;
    [newItem updateCurrentMiles:editCar.currentMiles];
  //  [newItem updateLeftTime];
    [editingCarMaintainItems addObject:newItem];
...
-(void) returnMainMenu
{
    ...
    /*
    if ([appDelegate.cars count]==0) {
        [appDelegate.tracingCarViewController loadCreateNewCarView];
    } */
...
{
    [appDelegate.maintainItemInitViewController setContainerData:editingCarMaintainItems containerTable:maintainItemsTable] ;

    [appDelegate loadMaintainItemInitView:YES ItemIndex:-editCar.currentMiles];
}
...
- (void)viewDidLoad
{
    ...
    oneCarInsuranceDate.delegate=self;
    oneCarInsuranceDate.inputView=appDelegate.tracingCarViewController.customInput;
    oneCarInsuranceDate.inputAccessoryView=appDelegate.tracingCarViewController.accessoryView;
    oneCarInspectDate.delegate=self;
    oneCarInspectDate.inputView=appDelegate.tracingCarViewController.customInput;
    oneCarInspectDate.inputAccessoryView=appDelegate.tracingCarViewController.accessoryView;
    insuranceValBtn.tag=inspectValBtn.tag=0;
    [inspectValBtn.layer setCornerRadius:5.0f];
    [insuranceValBtn.layer setCornerRadius:5.0f];
...
-(BOOL)checkEditInfo
{
    NSString *date;
    NSInteger interval;
    interval=inspectValBtn.tag;
    if (interval>0) {
        date=oneCarInspectDate.text;
        if ([date isEqualToString:@""]) {
            [appDelegate showNotification:@"车检日期有误！"];
            return NO;
        }
        editCar.inspectDay=oneCarInspectDate.text;
        editCar.inspectInterval=interval;
    }
    interval=insuranceValBtn.tag;
    if (interval>0) {
        date=oneCarInsuranceDate.text;
        if ([date isEqualToString:@""]) {
            [appDelegate showNotification:@"车险日期有误！"];
            return NO;
        }
        editCar.inspectDay=oneCarInsuranceDate.text;
        editCar.inspectInterval=interval;
    }
    return YES;
...
//    cell.itemDetail.text=[appDelegate.itemDetails objectAtIndex:item.itemId];
        cell.latestMaintainMiles.text=[NSString stringWithFormat:@"%ld", (long)item.latestMaintainMiles];
        cell.lifeMiles.text=[NSString stringWithFormat:@"%ld",(long)item.lifeMiles];
    ...
    if(buttonIndex==1)
    {
        if (alertView.tag==0) {
         ...
         //   [appDelegate.cars]
            ...
            //check concerned edited info.
            if (![self checkEditInfo]) {
                return;
            }
            [appDelegate updateCarInfo:editCar];
            MaintainItem *item;
            MaintainItemSetCell *cell;
            for (int i=0; i<[editingCarMaintainItems count]; i++) {
                cell=(MaintainItemSetCell*)[maintainItemsTable cellForRowAtIndexPath:[NSIndexPath indexPathForItem:i  inSection:0]];
                [cell takeOffKeyboard];
                item=[editingCarMaintainItems objectAtIndex:i];
                [item updateCurrentMiles:editCar.currentMiles];
              //  [item updateLeftTime];
            }
...
{
    if (textField==oneCarInsuranceDate) {
        appDelegate.tracingCarViewController.customInput.tag=3;
        appDelegate.currentInputDate=oneCarInsuranceDate;
        //  appDelegate.currentEditItem=item;
    }else if(textField==oneCarInspectDate){
        appDelegate.tracingCarViewController.customInput.tag=3;
        appDelegate.currentInputDate=oneCarInspectDate;
    }
}
